...
def compile_workflow_once(yaml_tree_ast: YamlTree,
    ...
    for i, step_key in enumerate(steps_keys):
        ...
        elif tool_i.cwl['class'] in ['Workflow', 'cwltool:ProcessGenerator']:
        ...
        if tool_i.cwl['class'] == 'cwltool:ProcessGenerator':
            # CWL clears all environment variables (e.g. HOME), so we need to specify homedir
            steps[i][step_key]['in']['homedir'] = args.homedir
            args_provided.append('homedir')

            # Add additional inputs to allow passing data to the inner workflow.
            # This lets us use runtime data at compile time!
            if 'inputs' in steps[i][step_key]:
                for key, val in steps[i][step_key]['inputs'].items():
                    if key not in tool_i.cwl['inputs']:
                        tool_i.cwl['inputs'][key] = val
                del steps[i][step_key]['inputs']

            # NOTE: It appears that outputs are not supported for ProcessGenerator
            # if 'outputs' in steps[i][step_key]:
            #     for key, val in steps[i][step_key].get('outputs', {}).items():
            #         if key not in tool_i.cwl['outputs']:
            #             # NOTE: Use 'run' to modify the outputs of the generated Process
            #             tool_i.cwl['run']['outputs'][key] = val
            #     del steps[i][step_key]['outputs']

            # Write out the modified ProcessGenerator
            run_path = f'{step_name_i}.cwl'
            if relative_run_path:
                run_path = ('../' * len(namespaces)) + run_path
            steps[i][step_key]['run'] = run_path
            with open('autogenerated/' + run_path, mode='w', encoding='utf-8') as f:
                f.write(yaml.dump(tool_i.cwl))
    ...
    # CWL separates input values into a separate file, which causes the input
    # variables in the workflow to be free/unbound variables. The minimal
    # cwltool:ProcessGenerator template we are using will not have those
    # variables, so we can re-bind them here using default values.
    # Any remaining variables that are actually free can be explicitly bound in
    # the parent yml file.
    if args.inline_inputs_file:
        for key, val in inputs_combined.items():
            if key in yaml_inputs:
                inputs_combined[key]['default'] = yaml_inputs[key]
