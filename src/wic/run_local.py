...
def run_local(args: argparse.Namespace, rose_tree: RoseTree, cachedir: Optional[str], cwl_runner: str, use_subprocess: bool) -> int:
    ...
    if cwl_runner == 'cwltool':
        ...
        cmd = [script] + docker_pull + parallel + quiet + cachedir_ + net + \
            write_summary + skip_schemas + path_check # + provenance + docker_cmd_
        cmd += ['--leave-outputs',
                '--singularity',
    ...
    if cwl_runner == 'toil-cwl-runner':
        ...
        cmd = ['toil-cwl-runner'] + docker_pull + net + path_check # + provenance + docker_cmd_
